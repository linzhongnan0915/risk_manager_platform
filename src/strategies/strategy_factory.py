...
from src.strategies.strategy_data import load_ohlcv_csv, ohlcv_long_to_panels
...
def _summary(
    ...
    return {
        ...
        "observations": len(daily), "cumulative_gross_return": float(daily["cumulative_gross"].iloc[-1]),
        "cumulative_net_return": float(daily["cumulative_net"].iloc[-1]), "gross_return_sum": gross_total,
        "transaction_cost_sum": cost_total, "cost_drag_ratio": float(cost_total / gross_total) if gross_total > 0 else None,
        "annualized_net_volatility": float(volatility(daily["net_return"])),
        "net_sharpe": float(sharpe_ratio(daily["net_return"])), "max_drawdown": float(max_drawdown(daily["net_return"])),
        "average_daily_turnover": float(daily["turnover"].mean()), "mean_ic": float(ic["ic"].mean()),
...
def _write_outputs(
    ...
    report = (
        ...
        "Signals are formed after close and positions execute next trading day open-to-close. "
...
def run_strategy(spec: StrategySpec, context: StrategyContext, output_dir: str | Path) -> dict[str, object]:
    ...
    ic, deciles = signal_diagnostics(score, eligible, asset_returns.shift(-execution_lag), rebalance_dates)
