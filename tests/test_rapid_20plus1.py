...
def test_platform_registry_counts():
    assert len(COMBINED_PORTFOLIO_MEMBER_IDS) == 20
    assert len(C3A1_IDS) == 17
    assert len(C3A2_IDS) == 9
    assert len(RAPID_BACKTEST_IDS) == 31
    assert COMPOSITE_ID == "COMBINED_PORTFOLIO_V1"
...
def test_combined_portfolio_matches_active_member_count():
    ...
    assert pytest.approx(summary["equal_weight"]) == pytest.approx(1.0 / active_count)
...
def test_bundle_has_twenty_eight_plus_composite():
