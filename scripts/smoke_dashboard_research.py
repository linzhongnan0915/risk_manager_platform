...
def main() -> int:
    ...
    if active != 14 or repair != 14 or archived != 10 or data_insufficient != 3 or reference != 18:
        errors.append("accepted final status counts do not match expected counts")
    if arch.get("composite_equal_weight") != 1 / 14:
        errors.append("Combined Portfolio equal weight must be 1/14")
